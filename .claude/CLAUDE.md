...
`/adversarial-review` skill in `.claude/skills/`, and the five agents in `.claude/agents/`.
...
- **Self-contained surface** — eleven commands in `.claude/commands/`, the `/adversarial-review` skill in `.claude/skills/` (single-artifact review, or `report` for a full sweep rendered as HTML), and five agents in `.claude/agents/` are the complete set. Do not invoke workflows, agents or artifact conventions from outside this repository.
